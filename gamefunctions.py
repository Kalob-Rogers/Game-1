...
# New save_game function for saving the player's progress
...
# New load_game function for loading the saved game data
...
def print_welcome(name: str, width: int = 20) -> None:
   # Corrected comment and added function definition as needed
    print(f"{'Hello, ' + name + '!':^{width}}")

def _shop_menu(item1Name: str, item1Price: float, item2Name: str, item2Price: float) -> None:
    border = "/" + "-" * 22 + "\\"
    item_format = "| {:12} ${:>6.2f} |"
    print(border)
    print(item_format.format(item1Name, item1Price))
...
def test_functions():
    ...
    _shop_menu("Apple", 2.5, "Banana", 1.2)
